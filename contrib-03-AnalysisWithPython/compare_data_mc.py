...
binning      = {'Dp_M' :np.arange(1600,2004,4), # LOWER BOUND, UPPER BOUND, BIN WIDTH
...
for var in vars_to_plot:
	...
	data_hist = df.query('Dp_M >1855 & Dp_M < 1885')[var].plot.hist(         bins=binning[var],log=True,alpha=0.5,label = '{0} Data'.format(var),normed=True,histtype='step')
	mc_hist   = df_cheat.query('Dp_M >1855 & Dp_M < 1885 & '+hlt2_cut_string)[var].plot.hist(              bins=binning[var],log=True,alpha=0.5,label = '{0}   MC'.format(var),normed=True,histtype='step')
    else: # USE A LOGARITHMIC SCALE FOR IPCHI2 AND FDCHI2
        data_hist = np.log(df.query('Dp_M >1855 & Dp_M < 1885'))[var].plot.hist( bins=binning[var],log=True,alpha=0.5,label = '{0} Data'.format(var),normed=True,histtype='step')
	mc_hist   = np.log(df_cheat.query('Dp_M >1855 & Dp_M < 1885 & '+hlt2_cut_string)[var]).plot.hist(      bins=binning[var],log=True,alpha=0.5,label = '{0}   MC'.format(var),normed=True,histtype='step')
    ...
    # ADD LEGEND
    plt.legend(handles=[data_patch,mc_patch],loc = 'upper left',prop={'size':10})
...
X_train_bkg =  df.query('Dp_M<1850 | Dp_M>1890')[features][int(0.2*n_events):n_events]
X_train_sig =  df_cheat.query(hlt2_cut_string)[features][int(0.2*n_events):n_events]
X_train     =  X_train_bkg.append(X_train_sig,ignore_index = True).values

# DEFINE WHICH PARTS OF TEST AND TRAINING SAMPLES CONTAIN SIGNAL AND WHICH BACKGROUND
...
plt.hist(bdt.decision_function(X_test_bkg).ravel(),color='r', alpha=0.5, range=(-0.4,0.4), bins=30)
plt.hist(bdt.decision_function(X_test_sig).ravel()  ,color='b', alpha=0.5, range=(-0.4,0.4), bins=30)
