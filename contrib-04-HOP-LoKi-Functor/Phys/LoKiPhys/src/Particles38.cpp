...
LoKi::Particles::BremMCorrected::clone() const { return new LoKi::Particles::BremMCorrected(*this); }


//Constructors
LoKi::Particles::BremMCorrected::BremMCorrected ( const double x , 
						  const double y , 
						  const double z )
  : LoKi::Particles::PtFlight ( x , y , z ) {} 
// ============================================================================ 
...
LoKi::Particles::PtFlightWithBestVertex::result_type
LoKi::Particles::BremMCorrected::operator() (const LoKi::Particles::BremMCorrected::argument p) const {


  std::vector <LHCb::Particle*> m_electrons;//needs initialization?                                                                                     
  std::vector <LHCb::Particle*> m_others;//needs initialization? 


  if ( 0 == p ) {
    Error ("Invalid argument, return 'InvalidMass'");
    return LoKi::Constants::InvalidMass ;
  }

  e_finder(p->clone(), m_electrons, m_others);

  LoKi::LorentzVector P_h_tot_nonconst(P_h_tot);
  LoKi::LorentzVector P_e_tot_nonconst(P_e_tot);
  LoKi::LorentzVector P_e_corr_tot_nonconst(P_e_corr_tot);
  double pt_h_nonconst = pt_h;
  double pt_e_nonconst = pt_e;

  for (const auto particle : m_others) {
    P_h_tot_nonconst += (particle->momentum()) ;
  }

  for (const auto particle : m_electrons) {
    P_e_tot_nonconst += (particle->momentum()) ;
  }

  pt_h_nonconst += ptFlight(P_h_tot, p->endVertex()->position(), position()) ;
  pt_e_nonconst += ptFlight(P_e_tot, p->endVertex()->position(), position()) ;

  double alpha = pt_h/pt_e;

  for (auto &particle : m_electrons) {
    LoKi::LorentzVector P_e_corr_temp_nonconst(P_e_corr_temp);
    double E_e = (pow((alpha * particle->momentum().X()), 2) + pow((alpha * particle->momentum().Y()), 2) + pow((alpha * particle->momentum().Z()), 2) + (m_e_PDG*m_e_PDG));
    P_e_corr_temp_nonconst.SetXYZT (alpha * particle->momentum().X(), alpha * particle->momentum().Y(), alpha * particle->momentum().Z(),
     E_e);
    P_e_corr_tot_nonconst += P_e_corr_temp_nonconst;
  }

  LoKi::LorentzVector P = P_h_tot_nonconst + P_e_corr_tot_nonconst;
...
void LoKi::Particles::BremMCorrected::e_finder (const LHCb::Particle *parent, 
						std::vector <LHCb::Particle*> m_electrons,
						std::vector <LHCb::Particle*> m_others) const {
  for (const auto &child : parent->daughtersVector()) {                                                                                               
    if (child->isBasicParticle()) {                                                                                                                   
      if (child->particleID().abspid() == 11)                                                                                                         
	m_electrons.push_back(child->clone()) ;                                                                                                       
      else                                                                                                                                            
	m_others.push_back(child->clone()) ;                                                                                                          
    } else {                                                                                                                                          
      if (has_electron(child)) e_finder(child, m_electrons, m_others) ;
  ...
  ( LoKi::Particles::BremMCorrectedWithBestVertex::argument p ) const 
{
  ...
  LorentzVector P_h_tot;
  LorentzVector P_e_tot;
  ...
  for (const auto child : (*m_electrons) ) {
